...
import { IdentityInput, AddressInput, GardenVectorOutput } from "../contracts/ContractAbi";
...
export default function Garden({ tileStates, contract, setTileStates, updateNum }: GardenProps) {
    useEffect(() => {
    ...
    return (
        <>
            {TILES.map((position, index) => (
                <GardenTile key={index} position={position} state={tileStates ? tileStates.inner[index] : undefined} updateNum={updateNum}/>
            )
            )}
        </>
    )
